...
use crate::{
    canonical,
    sliding_min::{
        Cache, sliding_lr_min_hash_mapper_scalar, sliding_lr_min_mapper_scalar,
        sliding_min_hash_mapper_scalar, sliding_min_mapper_scalar,
    },
};
...
use super::{
    ...
    sliding_min::{
        sliding_lr_min_hash_mapper_simd, sliding_lr_min_mapper_simd, sliding_min_hash_mapper_simd,
        sliding_min_mapper_simd,
    },
};
...
/// Like `minimizers_seq_scalar`, but returns the selected minimizer hash value directly.
#[inline(always)]
pub fn minimizer_hashes_seq_scalar<'s>(
    seq: impl Seq<'s>,
    hasher: &impl KmerHasher,
    w: usize,
    cache: &mut Cache,
    ...
    let len = kmer_hashes.len();
    kmer_hashes
        .map(sliding_min_hash_mapper_scalar::<true>(w, len, cache))
        .advance(w - 1)
}

/// Like `minimizers_seq_simd`, but returns the selected minimizer hash value directly.
#[inline(always)]
pub fn minimizer_hashes_seq_simd<'s>(
    seq: impl Seq<'s>,
    hasher: &impl KmerHasher,
    w: usize,
    cache: &mut Cache,
    ...
    let len = kmer_hashes.it.len();
    kmer_hashes
        .map(sliding_min_hash_mapper_simd::<true>(w, len, cache))
        .advance(w - 1)
}
...
#[inline(always)]
pub fn canonical_minimizer_hashes_seq_scalar<'s>(
    seq: impl Seq<'s>,
    hasher: &impl KmerHasher,
    w: usize,
    cache: &mut Cache,
    ...
    let delay1 = hasher.delay().0;
    let mut hash_mapper = hasher.in_out_mapper_scalar(seq);
    let mut sliding_min_mapper = sliding_lr_min_hash_mapper_scalar(w, seq.len(), cache);
    let (Delay(delay2), mut canonical_mapper) = canonical::canonical_mapper_scalar(k + w - 1);
...
#[inline(always)]
pub fn canonical_minimizer_hashes_seq_simd<'s>(
    seq: impl Seq<'s>,
    hasher: &impl KmerHasher,
    w: usize,
    cache: &mut Cache,
    ...
    let mut sliding_min_mapper = sliding_lr_min_hash_mapper_simd(w, padded_it.it.len(), cache);
    padded_it.advance_with(w - 1, |(a, rh, rc)| {
        let hash = hash_mapper((a, rh));
        canonical_mapper((a, rc));
        sliding_min_mapper(hash);
    ...
    padded_it.map(move |(a, rh, rc)| {
        let hash = hash_mapper((a, rh));
        let canonical = canonical_mapper((a, rc));
        let (lhash, rhash) = sliding_min_mapper(hash);
        canonical.blend(lhash, rhash)
    })
}
