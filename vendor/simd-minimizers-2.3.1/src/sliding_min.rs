...
pub struct Cache {
    ...
    scalar_hash: RingBuf<(u32, u32)>,
    scalar_lr_hash: RingBuf<((u32, u32), (u32, u32))>,
    simd: RingBuf<S>,
    simd_lr: RingBuf<(S, S)>,
    simd_hash: RingBuf<(S, S)>,
    simd_lr_hash: RingBuf<((S, S), (S, S))>,
}
...
/// Scalar version that returns the full hash value of the selected minimizer.
#[inline(always)]
pub fn sliding_min_hash_mapper_scalar<const LEFT: bool>(
    w: usize,
    len: usize,
    cache: &mut Cache,
) -> impl FnMut(u32) -> u32 {
    ...
    let mut prefix_min = (u32::MAX, u32::MAX);
    let ring_buf = cache.scalar_hash.assign(w, prefix_min);
    let val_mask = 0xffff_0000;
    let mut pos = 0;
    let max_pos = (1 << 16) - 1;

    fn min_pair<const LEFT: bool>(
        a @ (a_cmp, _): (u32, u32),
        b @ (b_cmp, _): (u32, u32),
    ) -> (u32, u32) {
        if (LEFT && b_cmp < a_cmp) || (!LEFT && b_cmp > a_cmp) {
            b
        } else {
            a
        }
    ...
    move |val| {
        ...
        let cmp = (if LEFT { val } else { !val } & val_mask) | pos;
        let elem = (cmp, val);
        pos += 1;
        ring_buf.push(elem);
        prefix_min = min_pair::<LEFT>(prefix_min, elem);
        if ring_buf.idx() == 0 {
            let mut suffix_min = ring_buf[w - 1];
            for i in (0..w - 1).rev() {
                suffix_min = min_pair::<LEFT>(suffix_min, ring_buf[i]);
                ring_buf[i] = suffix_min;
            }
            prefix_min = elem;
        }
        let suffix_min = unsafe { *ring_buf.get_unchecked(ring_buf.idx()) };
        min_pair::<LEFT>(prefix_min, suffix_min).1
    }
}
...
#[inline(always)]
pub fn sliding_lr_min_hash_mapper_scalar(
    w: usize,
    len: usize,
    cache: &mut Cache,
) -> impl FnMut(u32) -> (u32, u32) {
    ...
    let mut prefix_lr_min = ((u32::MAX, u32::MAX), (u32::MAX, u32::MAX));
    let ring_buf = cache.scalar_lr_hash.assign(w, prefix_lr_min);
    let val_mask = 0xffff_0000;
    let mut pos = 0;
    let max_pos = (1 << 16) - 1;

    fn lr_min_hash(
        (al, ar): ((u32, u32), (u32, u32)),
        (bl, br): ((u32, u32), (u32, u32)),
    ) -> ((u32, u32), (u32, u32)) {
        (
            std::hint::select_unpredictable(al.0 < bl.0, al, bl),
            std::hint::select_unpredictable(ar.0 > br.0, ar, br),
        )
    }
    ...
    move |val| {
        if pos == max_pos {
            let delta = ((1 << 16) - 2 - w) as u32;
            pos -= delta;
            prefix_lr_min.0.0 -= delta;
            prefix_lr_min.1.0 -= delta;
            for x in &mut **ring_buf {
                x.0.0 -= delta;
                x.1.0 -= delta;
            }
        }
        let lelem = ((val & val_mask) | pos, val);
        let relem = ((!val & val_mask) | pos, val);
        let elem = (lelem, relem);
        pos += 1;
        ring_buf.push(elem);
        prefix_lr_min = lr_min_hash(prefix_lr_min, elem);
        if ring_buf.idx() == 0 {
            let mut suffix_min = ring_buf[w - 1];
            for i in (0..w - 1).rev() {
                suffix_min = lr_min_hash(suffix_min, ring_buf[i]);
                ring_buf[i] = suffix_min;
            }
            prefix_lr_min = elem;
        }
        let suffix_lr_min = unsafe { *ring_buf.get_unchecked(ring_buf.idx()) };
        let (lmin, rmin) = lr_min_hash(prefix_lr_min, suffix_lr_min);
        (lmin.1, rmin.1)
    }
}
...
/// SIMD mapper that returns the full hash value of the selected minimizer.
pub fn sliding_min_hash_mapper_simd<const LEFT: bool>(
    w: usize,
    len: usize,
    cache: &mut Cache,
) -> impl FnMut(S) -> S {
    ...
    assert!(w < (1 << 15), "This method is not tested for large w.");
    assert!((len as u64) * 8 < (1u64 << 32));
    let mut prefix_min = (S::splat(u32::MAX), S::splat(u32::MAX));
    let ring_buf = cache.simd_hash.assign(w, prefix_min);
    let val_mask = S::splat(0xffff_0000);
    let max_pos = S::splat((1 << 16) - 1);
    let mut pos = S::splat(0);
    let delta = S::splat((1 << 16) - 2 - w as u32);
    ...
    move |val| {
        if pos == max_pos {
            reset_positions_offsets_hash(delta, &mut pos, &mut prefix_min, ring_buf);
        }
        let cmp = (if LEFT { val } else { !val } & val_mask) | pos;
        let elem = (cmp, val);
        pos += S::ONE;
        ring_buf.push(elem);
        prefix_min = simd_hash_min::<LEFT>(prefix_min, elem);
        if ring_buf.idx() == 0 {
            suffix_minima_hash::<LEFT>(ring_buf, w, &mut prefix_min, elem);
        }

        let suffix_min = unsafe { *ring_buf.get_unchecked(ring_buf.idx()) };
        simd_hash_min::<LEFT>(prefix_min, suffix_min).1
    }
}

#[inline(always)]
fn simd_hash_min<const LEFT: bool>((ac, ah): (S, S), (bc, bh): (S, S)) -> (S, S) {
    let use_b = if LEFT { bc.cmp_lt(ac) } else { ac.cmp_lt(bc) };
    (use_b.blend(bc, ac), use_b.blend(bh, ah))
}

fn suffix_minima_hash<const LEFT: bool>(
    ...
    let mut suffix_min = ring_buf[w - 1];
    for i in (0..w - 1).rev() {
        suffix_min = simd_hash_min::<LEFT>(suffix_min, ring_buf[i]);
        ring_buf[i] = suffix_min;
    }
    *prefix_min = elem;
}

fn reset_positions_offsets_hash(
    delta: S,
    pos: &mut S,
    prefix_min: &mut (S, S),
    ring_buf: &mut RingBuf<(S, S)>,
...
/// Like `sliding_lr_min_mapper_simd`, but returns the full hash values of both selected minima.
pub fn sliding_lr_min_hash_mapper_simd(
    w: usize,
    len: usize,
    cache: &mut Cache,
) -> impl FnMut(S) -> (S, S) {
    ...
    assert!(w < (1 << 15), "This method is not tested for large w.");
    assert!((len as u64) * 8 < (1u64 << 32));
    let mut prefix_lr_min = (
        (S::splat(u32::MAX), S::splat(u32::MAX)),
        (S::splat(u32::MAX), S::splat(u32::MAX)),
    );
    let ring_buf = cache.simd_lr_hash.assign(w, prefix_lr_min);
    let val_mask = S::splat(0xffff_0000);
    let max_pos = S::splat((1 << 16) - 1);
    let mut pos = S::splat(0);
    let delta = S::splat((1 << 16) - 2 - w as u32);
    ...
    move |val| {
        if pos == max_pos {
            reset_positions_offsets_lr_hash(delta, &mut pos, &mut prefix_lr_min, ring_buf);
        }
        let lelem = ((val & val_mask) | pos, val);
        let relem = ((!val & val_mask) | pos, val);
        let elem = (lelem, relem);
        pos += S::ONE;
        ring_buf.push(elem);
        prefix_lr_min = simd_lr_hash_min(prefix_lr_min, elem);
        if ring_buf.idx() == 0 {
            suffix_lr_minima_hash(ring_buf, w, &mut prefix_lr_min, elem);
        }

        let suffix_lr_min = unsafe { *ring_buf.get_unchecked(ring_buf.idx()) };
        let (lmin, rmin) = simd_lr_hash_min(prefix_lr_min, suffix_lr_min);
        (lmin.1, rmin.1)
    }
}
...
#[inline(always)]
fn simd_lr_hash_min(
    ((alc, alh), (arc, arh)): ((S, S), (S, S)),
    ((blc, blh), (brc, brh)): ((S, S), (S, S)),
) -> ((S, S), (S, S)) {
    let use_bl = blc.cmp_lt(alc);
    let use_br = arc.cmp_lt(brc);
    (
        (use_bl.blend(blc, alc), use_bl.blend(blh, alh)),
        (use_br.blend(brc, arc), use_br.blend(brh, arh)),
    )
}

#[inline(always)]
fn suffix_lr_minima_hash(
    ring_buf: &mut RingBuf<((S, S), (S, S))>,
    w: usize,
    prefix_min: &mut ((S, S), (S, S)),
    elem: ((S, S), (S, S)),
) {
    unsafe { assert_unchecked(ring_buf.len() == w) };
    unsafe { assert_unchecked(w > 0) };
    let mut suffix_min = ring_buf[w - 1];
    for i in (0..w - 1).rev() {
        suffix_min = simd_lr_hash_min(suffix_min, ring_buf[i]);
        ring_buf[i] = suffix_min;
    }
    *prefix_min = elem;
}
...
#[inline(always)]
fn reset_positions_offsets_lr_hash(
    delta: S,
    pos: &mut S,
    prefix_lr_min: &mut ((S, S), (S, S)),
    ring_buf: &mut RingBuf<((S, S), (S, S))>,
) {
    *pos -= delta;
    prefix_lr_min.0.0 -= delta;
