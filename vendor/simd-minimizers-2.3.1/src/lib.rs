...
use minimizers::{
    canonical_minimizer_hashes_seq_scalar, canonical_minimizer_hashes_seq_simd,
    canonical_minimizers_seq_scalar, canonical_minimizers_seq_simd, minimizer_hashes_seq_scalar,
    minimizer_hashes_seq_simd, minimizers_seq_scalar, minimizers_seq_simd,
};
use packed_seq::Seq;
    ...
    Builder<'h, CANONICAL, H, &'o2 mut Vec<u32>, 0>
{
    ...
    pub fn run_hashes_scalar<'s, SEQ: Seq<'s>>(self, seq: SEQ, minimizer_hashes: &mut Vec<u32>) {
        let default_hasher = self.hasher.is_none().then(|| H::new(self.k));
        let hasher = self
            .hasher
            .unwrap_or_else(|| default_hasher.as_ref().unwrap());
        ...
        CACHE.with_borrow_mut(|cache| match CANONICAL {
            false => collect_and_dedup_with_index_into_scalar(
                minimizer_hashes_seq_scalar(seq, hasher, self.w, &mut cache.0),
                minimizer_hashes,
                self.sk_pos,
            ),
            true => collect_and_dedup_with_index_into_scalar(
                canonical_minimizer_hashes_seq_scalar(seq, hasher, self.w, &mut cache.0),
                minimizer_hashes,
                self.sk_pos,
            ),
        });
    ...
    pub fn run_hashes<'s, SEQ: Seq<'s>>(self, seq: SEQ, minimizer_hashes: &mut Vec<u32>) {
        CACHE.with_borrow_mut(|cache| self.run_hashes_with_buf(seq, minimizer_hashes, &mut cache.0))
    }
    ...
    #[inline(always)]
    fn run_hashes_with_buf<'s, SEQ: Seq<'s>>(
        self,
        seq: SEQ,
        minimizer_hashes: &mut Vec<u32>,
        cache: &mut Cache,
    ) {
        let default_hasher = self.hasher.is_none().then(|| H::new(self.k));
        let hasher = self
        ...
        match CANONICAL {
            false => minimizer_hashes_seq_simd(seq, hasher, self.w, cache)
                .collect_and_dedup_with_index_into(minimizer_hashes, self.sk_pos),
            true => canonical_minimizer_hashes_seq_simd(seq, hasher, self.w, cache)
                .collect_and_dedup_with_index_into(minimizer_hashes, self.sk_pos),
        };
    }
}
...
impl<'s, 'o, const CANONICAL: bool, SEQ: Seq<'s>> Output<'o, CANONICAL, SEQ> {
    /// Iterator over hash values associated with all minimizer positions.
    #[must_use]
    pub fn hashes_u32<'h, H: KmerHasher>(
        &'h self,
        hasher: &'h H,
    ) -> impl ExactSizeIterator<Item = u32> + 'h {
        self.min_pos.iter().map(
            #[inline(always)]
            move |&pos| hasher.hash_seq(self.seq.slice(pos as usize..pos as usize + self.len)),
        )
    }
