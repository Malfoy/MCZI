...
impl GGCATInstance {
    ...
    pub fn create(config: GGCATConfig) -> anyhow::Result<&'static Self> {
        ...
        let rayon_result = rayon::ThreadPoolBuilder::new()
            .num_threads(config.total_threads_count)
            .thread_name(|i| format!("rayon-thread-{}", i))
            .build_global();
        if let Err(err) = rayon_result {
            let message = err.to_string();
            if !message.contains("already been initialized") {
                anyhow::bail!("Cannot initialize rayon thread pool: {}", message);
            }
        }
